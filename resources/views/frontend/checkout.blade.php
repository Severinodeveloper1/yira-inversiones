...
@section('content')
    <main class="pt-12 pb-24 max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop min-h-screen">
        ...
        <header class="mb-12">
            ...
            <p class="text-secondary font-body-md">Complete los datos de facturación y despacho para generar su comprobante
                de compra.</p>
        </header>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12" id="checkout-container">
                ...
                <form id="checkout-form" class="space-y-8">
                    ...
                    <div class="bg-surface-container-lowest p-8 shadow-sm rounded-lg border border-surface-variant/20">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <label
                                class="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-md cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all"
                                id="label-boleta">
                                <input type="radio" name="document_type" value="boleta" checked class="sr-only"
                                    onchange="toggleDocType('boleta')">
                            ...
                            <label
                                class="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-md cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all"
                                id="label-factura">
                                <input type="radio" name="document_type" value="factura" class="sr-only"
                                    onchange="toggleDocType('factura')">
                            ...
                            <label
                                class="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-md cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all"
                                id="label-ticket">
                                <input type="radio" name="document_type" value="ticket" class="sr-only"
                                    onchange="toggleDocType('ticket')">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="doc_type_select" id="doc_type_select"
                                        class="w-full px-4 py-3 bg-surface border border-outline/20 focus:border-primary focus:ring-0 outline-none rounded transition-all"
                                        onchange="updateDocFieldLabel()">
                                        <option value="DNI" {{ $customer?->document_type == 'DNI' ? 'selected' : '' }}>DNI
                                        </option>
                                        <option value="RUC" {{ $customer?->document_type == 'RUC' ? 'selected' : '' }}>RUC
                                        </option>
                                        <option value="CE" {{ $customer?->document_type == 'CE' ? 'selected' : '' }}>Carnet de
                                            Extranjería</option>
                                ...
                                <div>
                                    <label
                                        class="font-label-sm text-label-xs uppercase tracking-wider text-secondary block mb-1"
                                        id="doc-number-label">Número de Documento *</label>
                                    <input type="text" name="document_number" value="{{ $customer?->document_number }}"
                                        required
                                        class="w-full px-4 py-3 bg-surface border border-outline/20 focus:border-primary focus:ring-0 outline-none rounded transition-all">
                                </div>
                            </div>
                    ...
                    <div class="bg-surface-container-lowest p-8 shadow-sm rounded-lg border border-surface-variant/20">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            <label
                                class="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-md cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all"
                                id="label-recojo">
                                <input type="radio" name="shipping_method" value="recojo_tienda" checked class="sr-only"
                                    onchange="toggleShipping('recojo_tienda')">
                            ...
                            <label
                                class="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-md cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all"
                                id="label-delivery">
                                <input type="radio" name="shipping_method" value="delivery" class="sr-only"
                                    onchange="toggleShipping('delivery')">
                            ...
                            <label
                                class="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-md cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all"
                                id="label-agencia">
                                <input type="radio" name="shipping_method" value="envio_agencia" class="sr-only"
                                    onchange="toggleShipping('envio_agencia')">
                        ...
                        <div id="shipping-address-container" class="hidden transition-all duration-300">
                            <label
                                class="font-label-sm text-label-xs uppercase tracking-wider text-secondary block mb-1">Dirección
                                de Envío y Agencia *</label>
                            <textarea name="shipping_address" id="shipping_address"
                                class="w-full px-4 py-3 bg-surface border border-outline/20 focus:border-primary focus:ring-0 outline-none rounded transition-all h-28"
                                placeholder="Ingrese la dirección completa. Para agencias, especifique el nombre de la agencia (ej. Flores, Shalom, Olva Courier) y departamento de destino..."></textarea>
                        </div>
                    </div>

                    <!-- Submit Button Container -->
                    <button type="submit" id="submit-btn"
                        class="w-full bg-primary text-white py-5 rounded font-bold font-body-lg hover:bg-primary-container transition-colors shadow-lg flex items-center justify-center gap-2 uppercase tracking-widest active:scale-[0.98]">
            ...
            <aside class="lg:col-span-5">
                <div
                    class="bg-surface-container-lowest p-8 shadow-sm rounded-lg sticky top-28 border border-surface-variant/20 space-y-6">
                    <h2 class="font-headline-lg text-[22px]">Resumen del Pedido</h2>

                    <!-- Items list review -->
                    <div id="checkout-items-list"
                        class="max-h-72 overflow-y-auto divide-y divide-surface-variant/40 pr-2 hide-scroll">
                    ...
                    <div class="pt-6 border-t border-surface-variant">
                        <div class="flex justify-between items-baseline">
                            ...
                            <span class="font-headline-xl text-3xl text-primary font-bold" id="checkout-total">S/.
                                0.00</span>
                        </div>
                    </div>
                </div>
